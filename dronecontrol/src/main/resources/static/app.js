...
function connect() {
    ...
    stompClient.connect({}, function (frame) {
        ...
        console.log("reset...");
        stompClient.subscribe('/chat/room/1', function (data) {
                console.log(91, data);
        });
        ...
        stompClient.send("/app/room/greeting/1", {}, DRONE_ID,
        );
    });
}
...
function getCurrentLocation() {
    ...
    string_mission = "";
    removePushpin();
    stompClient.send("/app/clearmission", {}, DRONE_ID,
        );
    current_loc = null;
    current_pin = null;
    stompClient.send("/app/getLocation", {}, DRONE_ID,
        );
}
...
function executeTakeoff() {
    sendClear();
    stompClient.send("/app/takeoffandland", {}, DRONE_ID,);
    location_data = [];
    pin_data = [];
...
function uploadMission(loca) {
    ...
    if (check_mission == count_mission) {
        ...
        stompClient.send("/app/uploadmission/", {}, JSON.stringify({stringMission, droneId : DRONE_ID}));
        setTimeout(function () {
...
function executeMisson() {
    stompClient.send("/app/runmissionrtl", {}, DRONE_ID,);
    location_data = [];
    pin_data = [];
...
function executeMissonNorl() {
    stompClient.send("/app/runmission", {}, DRONE_ID,);
    location_data = [];
    pin_data = [];
...
function stopMission() {
	stompClient.send("/app/stopmission", {}, DRONE_ID);
...
$(function () {
    ...
    $("#button-stopMission").click(function () {
        stopMission();
    });
